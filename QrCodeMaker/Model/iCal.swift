...
public enum iCal {
    ...
    public static func load(string: String) -> [Calendar1] {
    ...
    public static func load(url: URL, encoding: String.Encoding = .utf8) throws -> [Calendar1] {
        let data = try Data(contentsOf: url)
        guard let string = String(data: data, encoding: encoding) else { throw iCalError.encoding }
        return load(string: string)
    }

    public static func parse(_ icsContent: [String]) -> [Calendar1] {
